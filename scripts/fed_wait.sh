...
#
# Robust busy markers for Claude, Codex, and Hermes. Keep this default in sync
# with fed_sessions.sh; override FED_BUSY_RE only for local debugging.
...
for ((i=1; i<=MAXIT; i++)); do
  ...
  anybusy=0
  for s in "${sessions[@]}"; do
    pane="$(tmux capture-pane -J -t "$s" -p -S -12 2>/dev/null)" || {
